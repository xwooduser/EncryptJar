 ...
 */
public class rrname {
	private static Log log = LogFactory.getLog(rrname.class);
	String classpath;
	String _classpath;

	{
		_classpath = rrname.class.getClassLoader().getResource(".")
				.toString();
	...
	rrname() {
		classpath = rrname.class.getClassLoader().getResource(".")
				.toString();
		classpath = classpath.replaceAll("file:", "");
		classpath = classpath.substring(1, classpath.length());
	}

	rrname(String classpath) {
	...
	*/
	...
	if("".equals(str.trim())||str==null)
			str=_classpath;
	try{	
		if((str.indexOf(".jar") != -1||str.indexOf(".JAR") != -1)){
			 System.out.println("********1111*************");
			//System.out.println("1111111111111111111111111112222222222222222");
	...
	public void renameFile(String _path,String fileName) throws Exception{
		...
		// String oFileName=rJarName(fileName)+String.valueOf(Math.random()).substring(2,8)+".jar";
		  String oFileName=rrJarName(fileName)+".jar";
		 System.gc();
	...
	public void renameFolder(String _path) throws Exception{			
		...
		for(String f:flist){
			...
			if(!f2.isDirectory()&&(f.indexOf(".jar")!=-1||f.indexOf(".JAR")!=-1)){
				log.info("正在还原文件:"+_path+f);				 
				renameFile(_path,f);
	...
	void compileAll(String inputComment) {				 
		...
		if (inputComment.indexOf(".jar") != -1||inputComment.indexOf(".JAR") != -1) {
	...
	void compileClass(String fPath){		
		try {
			...
			if((fPath.indexOf("/")!=-1||fPath.indexOf("\\")!=-1)&&(fPath.indexOf(".jar")!=-1||fPath.indexOf(".JAR")!=-1)){
				_fpath=fPath;
	...
	{		 
		rrname rcc=new rrname();
		System.out.println("服务器已启动...");
		System.out.print("请输入还原命令：");
	...
	//			  System.out.print((char)c); 
				  if((char)c=='\n'){
					  System.out.println("还原准备中...");				  
					  rcc.inputLine(str.toString().trim());
					  break;
				  }
			}			
		}catch(Exception e) {
			  System.out.println("还原出错："+e.getMessage());
			  e.printStackTrace();
		}
		System.out.println("");
